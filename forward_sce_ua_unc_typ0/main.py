...
def calc_sceua(i, df, vs, ms):
    ...
    hourly_obs['datetime'] = pd.to_datetime(hourly_obs[['year', 'month', 'day', 'hour']])
    hourly_obs.set_index('datetime', inplace=True)
    daily_obs = hourly_obs.resample('D').agg({
        'year': 'first',
        'doy': 'mean',
        'ta': 'mean',
        'nee': 'mean',
    ...
    spot = spot_setup(root, site_ID, pft, vs, ms, hourly_obs, daily_obs)

    sampler = spotpy.algorithms.sceua(spot, dbname=f'{site_ID}_SCEUA', dbformat='csv', parallel='mpi', save_sim=False)
    rep = 200000

...
"""
import numpy as np
from tbm.constants import T2K

from tbm.RTM_Optical import rtm_o, BRF_hemi_dif_func
from tbm.Ebal import Ebal
from tbm.PhotoSynth_Jen import PhotoSynth_Jen, calc_resp
from tbm.hydraulics import calc_hy_f
...
class TBM_Model():
    ...
    def tbm(self, pd, ph):
        ...
        for h in range(0, 24):
            ...
            Year = self.d.year[xh] # Year
            Doy = self.d.D[xh]  # day of year
            hour = self.d.hour[xh]  # hour of year

            An_daily += An

            nee = -An + (self.p.theta_lit * cl + self.p.theta_som * cs) * self.temp_term(self.p.Theta, self.d.t_mean[xh])
            ph_out[h] = [Year, Doy, hour, nee, rtm_o_dict['fPAR'], rtm_o_dict['sur_refl_red'], rtm_o_dict['sur_refl_nir'], Ebal_dict['LST']]
        ...
        if self.d.D[self.x * 24] > (self.p.d_fall + self.p.crfall / 2.0):
            clab2 += cf
            cf2 = 0.0
        else:
        ...
        Year = self.d.year[self.x * 24] # Year
        Doy = self.d.D[self.x * 24]  # day of year
        hour = self.d.hour[self.x * 24]  # hour of year

        pd_out[:] = [clab2, cf2, cr2, cw2, cl2, cs2, Year, Doy, hour, gpp, lai]
    ...
    def mod_list(self, output_dim1, output_dim2):
        ...
        mod_list_spectral = np.full((self.endrun - self.startrun + 1, 24, 2101), 0.0)
        self.x = self.startrun
        for t in xrange(self.endrun - self.startrun):
            mod_list_daily[(t + 1)], mod_list_hourly[(t + 1)], mod_list_spectral[(t + 1)] = self.tbm(mod_list_daily[t],
                                                                                                     mod_list_hourly[t])
            self.x += 1
